...
"text!search-interface/search-results-list.hbs", "text!options/modal.hbs",
"search-interface/variable-info-modal-view", "search-interface/search-util",
...
"search-interface/keyboard-nav", ], function ( BB, HBS,
    searchResultsViewTemplate, searchResultsListTemplate, modalTemplate,
    dataTableInfoView, searchUtil, numericFilterModalView,
    categoricalFilterModalView, filterModel, tagFilterModel, modal,
    variableInfoCache, keyboardNav ) {
        let StudyResultsView = BB.View.extend({
            ...
            nextPage: function () {
                let nextPageLink = document.getElementById(
                    "page-link-" + tagFilterModel.get("currentPage")
                ).nextElementSibling;
                if (nextPageLink) {
                ...
                $("#aria-live").html(
                    "Now on page " +
                    tagFilterModel.get("currentPage") +
                    " of the results region."
                );
            },
            previousPage: function () {
                let previousPageLink = document.getElementById(
                    "page-link-" + tagFilterModel.get("currentPage")
            ...
            resultsBlur: function () {
                ...
                this.$(
                    "#search-results-div  .search-result.focused-search-result"
                ).removeClass("focused-search-result");
            },
            cacheVariableInfo: function (response, variableId) {
                variableInfoCache[variableId] = {
                    ...
                    studyAccessionTagId: this.generateStudyAccessionTagId(
                        response.metadata.study_id
                    ),
                    studyAccessionTagName: searchUtil.findStudyAbbreviationFromId(
                        response.metadata.study_id
                    ),
            ...
            retrieveDataTableMeta: function (dataTableId, successHandler) {
                $.ajax({
                    ...
                    data: JSON.stringify({
                        ...
                        query: { id: dataTableId, entityType: "DATA_TABLE" },
                    }),
            ...
            render: function () {
                ...
                if (tagFilterModel.get("searchResults")) {
                    let results = _.map(
                        tagFilterModel.get("searchResults").results.searchResults,
                        function (result, i) {
                            let metadata = result.result.metadata;
                            return {
                                abbreviation: searchUtil.findStudyAbbreviationFromId(
                                    metadata.study_id
                                ),
                    ...
                    for (
                        var offset = 0;
                        offset < tagFilterModel.get("searchResults").results.numResults;
                        offset += pageSize
                    ) {
                    ...
                    $("#search-results-div").html(
                        HBS.compile(searchResultsListTemplate)({
                            results: results,
                            variableCount: tagFilterModel.get("searchResults").results
                            .numResults,
                            pages: pages,
                        })
                    );
                    $("#search-results-div").attr(
                        ...
                        tagFilterModel.get("searchResults").results.numResults +
