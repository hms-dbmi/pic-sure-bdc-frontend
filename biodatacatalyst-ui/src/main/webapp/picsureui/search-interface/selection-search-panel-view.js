define(['jquery', 'backbone','handlebars', 'text!search-interface/selection-search-panel.hbs'],
    function($, BB, HBS, searchPanelTemplate) {
        const updatedEvent = new Event('updatedLists')
        let selectionSearchView = BB.View.extend({
            initialize: function(opts){
                if (opts && opts.heading) {
                    ...
                    opts.sample ? this.data.placeholder = _.sample(opts.results, 10) : this.data.placeholder = opts.results;
            ...
            // Fix this to use index or something else
            addEvents: function() {
                // $('.genomic-value-container.selection-search-results input').on('click', this.selectItem.bind(this));
            ...
            renderLists: function() {
                const newHTMLList  = this.data.placeholder.map((item) => {
                    ...
                    `<input class="categorical-filter-input"  type="checkbox" value="${item}" checked disabled/>${item}<br/>` : 
                    `<input class="categorical-filter-input"  type="checkbox" value="${item}" />${item}<br/>`;
                }, this);
                const newHTMLRSelectionList  = this.data.selectedResults.map((item) => {
                    return `<input class="categorical-filter-input" type="checkbox" value="${item}" checked/>${item}<br/>`;
                });      
